...
function ReadFilesAndPostOrderXMLs(n, HTMLs) {
  for(var d = n; d < n+CONFIG.FILES_PER_A_THREAD; d++){
    if (d < HTMLs.length) {
      var StringofHTml = '';
      fs.readFile(CONFIG.HTML_INDEX_DIR + HTMLs[d], function (err, data) {
        if (!err) {
          try {
          ...
          logger.log(CONFIG.HTML_INDEX_DIR, "Can't read: " + HTMLs[d], "");
        }
      });
      if(d == n+CONFIG.FILES_PER_A_THREAD-1){
        ReadFilesAndPostOrderXMLs(d + 1, HTMLs);
      }
    }
    else {
